...
- **TypeScript:** the repo is converting to TypeScript feature by feature (`authentication` and `clubs` are done — see [docs/specs/README.md](./docs/specs/README.md#typescript) and [docs/specs/architecture.md](./docs/specs/architecture.md#typescript)). Every feature has (or should have) a `types/` folder colocated at `src/features/<name>/types/` (or `src/types/` for types genuinely shared across features). Inside that folder, split declarations by kind into separate files rather than one combined `index.ts`:
