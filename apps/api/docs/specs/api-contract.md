...
| `GET /events` | events | no | `eventEndpoints.all` | the public `Events.tsx` directory still renders hardcoded data (the `getEvents()` fetcher exists and is uncalled), but the `host=` branch below **is** live |
| `GET /events?host={userName}` | events | no | `eventEndpoints.byHost` | ✅ `YourEvents.tsx` (`/organization/events`) — one host's own events, filtered server-side on `hostUsername` |
