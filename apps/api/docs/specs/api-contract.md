...
| `GET /organizations` | organizations | no | `organizationEndpoints.all`, `organizationEndpoints.directory({...})`, `organizationEndpoints.details(userName)` | ✅ `Organizations.tsx`'s SWR key (the live directory, filtered server-side via `?search=&domain=`). The unfiltered branch now lists **live organization records**, not organization-type users — see [organizations.md](./organizations.md). The `?userName=` branch is unchanged and still answers out of `users` (`Profile.tsx`). `GetAllOrganizations()` still exists and is still only reachable from the unrouted `Donate.tsx` |
| `GET /organizations/taxonomy` | organizations | no | `organizationEndpoints.taxonomy` | ✅ `Organizations.tsx` (filter chips) and `OrganizationSetup.tsx` (tag select + domain chips) |
| `GET /organizations/me` | organizations | `requireAuth` | `organizationEndpoints.mine` | ✅ `OrganizationSetup.tsx`'s SWR key |
| `PATCH /organizations/me` | organizations | `requireAuth` | `organizationEndpoints.mine` | ✅ `UpdateMyOrganization()` |
| `GET /organizations/{handle}` | organizations | no | `organizationEndpoints.byHandle(handle)` | ✅ `OrganizationProfile.tsx`'s SWR key. `404`s for a draft organization exactly as for an unknown handle |
...
| `GET /events` | events | no | `eventEndpoints.directory()`, `eventEndpoints.byUid()` | ✅ live on both branches: `Events.tsx`'s directory grid (`limit=100`, no `uid`) and, September 2026, `DetailedEvent.tsx`'s single-event fetch (`?uid={id}`, reusing this same route rather than a dedicated `GET /events/:uid` — see [events.md](./events.md)). The unused `eventEndpoints.all`/`getEvents()` fetcher noted elsewhere is a separate, still-dead code path, not this one |
| `GET /events?host={handle}` | events | no | `eventEndpoints.byHost` | ✅ `YourEvents.tsx` (`/organization/events`) and, new in September 2026, `OrganizationProfile.tsx`'s "Events hosted" section — one host's own events, matched server-side against **either** `organizerHandle` or the legacy `hostUsername` (see [events.md](./events.md)) |
...
| `POST /payment/organizations/{handle}/order` | payments | no | `paymentEndpoints.sponsorshipOrder(handle)` | ✅ `CreateSponsorshipOrder()`, called from `SponsorOrganizationModal.tsx` — see [payments.md](./payments.md#the-organization-sponsorship-flow--order-model-september-2026) |
| `POST /payment/organizations/{handle}/verify` | payments | no | `paymentEndpoints.sponsorshipVerify(handle)` | ✅ `VerifySponsorshipPayment()`, called from the same modal's Razorpay Checkout `handler` |
