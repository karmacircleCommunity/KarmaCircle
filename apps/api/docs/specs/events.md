...
**`host`** (August 2026) narrows that list to one host's own events, matched against each event's `hostUsername`. It backs the frontend's `/organization/events` page; the filter is server-side precisely so a host with more events than one page still sees all of them (see the frontend's [events.md](../../../../docs/specs/events.md)). The `slug` alias exists because the frontend's `eventEndpoints`/route naming uses "slug" in some places and "uid" in others for what is the same field on this side — see [api-contract.md](./api-contract.md#get-events) before assuming which query param name a given frontend call site actually sends.
