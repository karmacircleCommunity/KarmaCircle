...
`auth`, `events`, `products`, and `users` have test files (`tests/auth.test.ts`, `tests/events.test.ts`, `tests/products.test.ts`, `tests/users.test.ts`). `clubs`, `directory`, `payments`, and `reports` are completely untested — a change to any of them is only checked by `typecheck`/`lint`, not by CI-run behavioral tests, until coverage is added. See [architecture.md](./architecture.md#testing).
