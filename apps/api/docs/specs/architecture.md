...
`origin` is a callback that checks the caller against a real allowlist, built once at startup as `new Set([env.ORIGIN_URL, ...env.CORS_ORIGINS])`.
A request whose `Origin` is on the list gets that origin echoed back; anything else gets no `Access-Control-Allow-Origin` header at all, which is how a CORS denial is expressed (the browser blocks the response).
Requests with no `Origin` header, such as curl or server-to-server calls, are allowed through, since CORS is a browser mechanism and there is no browser origin to protect.
`credentials: true` is required for the frontend's `withCredentials: true` axios calls to send and receive the `Token` cookie, and it also means a `*` wildcard is never a legal answer here, so the list has to be genuine.

This replaced a callback that always answered with the single configured `ORIGIN_URL` no matter who asked, plus an `IGNORE_ORIGINS` flag that allowed **every** origin.
That pairing had no way to express "these two specific origins", so local development ran on a wildcard CORS policy.
`CORS_ORIGINS` is a comma-separated list of *additional* origins; `ORIGIN_URL` is always allowed implicitly, so a typical deployment leaves `CORS_ORIGINS` empty.

## Config (`src/config/`)

- **`env.ts`** — a Zod schema (`envSchema`) validates `process.env` once at module load; `loadEnv()` throws a formatted multi-line error (one bullet per bad/missing var) if validation fails, so the process refuses to start with a broken config rather than surfacing `undefined` deep in a request handler later. `PORT` defaults to `5000`; `MONGO_URI`, `JWT_SECRET`, `SECRET_KEY`, `CLIENT_ID`, `CLIENT_SECRET`, `CALLBACK_URL`, `successURL` and `ORIGIN_URL` are required with no default. `ORIGIN_DOMAIN` is **optional**, and leaving it unset is meaningful rather than lazy: it makes auth cookies host-only, which is what localhost needs (see [auth.md](./auth.md)). `CORS_ORIGINS` is optional too, and transforms from a comma-separated string into a trimmed array. `SECRET_KEY` is validated as required but **not read anywhere else in the codebase** — grep confirms only `env.ts` references it; likely vestigial (perhaps once used for `express-session` or a different signing scheme) — don't assume it does anything today.
...
See [testing.md](./testing.md) — the full picture (Jest+Supertest's setup, the per-module coverage table, and the separate standalone e2e server `apps/web`'s Playwright suite drives) now lives there. Every module has test coverage as of September 2026.
