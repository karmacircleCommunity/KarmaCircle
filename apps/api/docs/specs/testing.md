...
- **A test-only password-reset outbox.** [config/mailer.ts](../../src/config/mailer.ts)'s `sendPasswordResetEmail` never calls the real Resend API when `env.NODE_ENV === "test"` — it pushes `{ to, resetUrl }` into an in-memory array instead (a fake `RESEND_API_KEY` would otherwise fail every real send). `GET /__test__/last-reset-url?email=...` (also `NODE_ENV==="test"`-gated, in `app.ts`) reads the most recent entry for an address back out, 404ing if none exists. `POST /__test__/reset` clears the outbox too (`clearTestOutbox()`), alongside wiping the database. This is what lets `apps/web/e2e/authentication/password-reset.spec.ts` complete a real forgot→reset→sign-in-with-the-new-password round trip without a real mail provider — Jest never touches any of this, since `tests/auth.test.ts` `jest.mock()`s the whole `mailer` module instead (same pattern as `payments.test.ts` and `razorpay` — see above).
