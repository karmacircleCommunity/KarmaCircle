...
**API/network error feedback is per-call, via toasts, not a global mechanism.** Every `KarmaCircleApi.ts` function catches its own errors and returns a response-shaped value rather than throwing (see [api-integration.md](../../../docs/specs/api-integration.md)); callers are individually responsible for checking `response?.status` and calling `showErrorToast(...)`. There is no interceptor-level (e.g. Axios response interceptor) centralized error handling anywhere in `src/services/` — each of the roughly dozen `KarmaCircleApi.ts` functions repeats its own `try/catch`.
