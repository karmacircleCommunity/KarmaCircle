...
- **"Add SEO tags to the 404 page"** → follow the `<Helmet>` pattern used by `Home.tsx`/`Auth.tsx` (simplest, most consistent with a page that has no `type`-keyed `ComponentHelmet` branch of its own).
