...
3. **Password.** Branches on `authType`:
   - `authType === "signin"`: only checks `credentials.password` is non-empty, setting `errors.password = "Password is required"` on failure. Nothing else about the value is inspected client-side — the actual credential is validated by the backend's `LoginUser` response (see step 4). This is a deliberate split, not an oversight: password *strength* is a creation-time concept, and reusing the sign-up rule here used to reject valid logins from any account whose password predated that rule (or simply didn't match the pattern), before any network call was even made.
...
- **`useAuth.ts`'s email check still re-runs on sign-in** (not just sign-up) — legitimate, since a malformed email can't be a valid login identifier either. The password *strength* check no longer runs on sign-in (fixed — see `hooks/useAuth.ts` above): sign-in now only requires a non-empty password and leaves credential correctness to the backend.
