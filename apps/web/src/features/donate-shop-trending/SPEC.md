...
- **Login gate (fixed August 2026):** now `if (!isLoggedIn) { toast.error(...); navigate("/auth/signin"); }`, where `isLoggedIn` comes from Redux via `selectIsLoggedIn`.
  It previously redirected to `/user/login`, which is not a route in `routesConfig.tsx` and landed on the 404 page, and gated on a `Cookies.get("isLoggedIn")` value that the frontend could never actually read.
  The API has since stopped setting readable identity cookies entirely (see [apps/api auth.md](../../../../../apps/api/docs/specs/auth.md)), so Redux is the only source here, matching the rest of the app.
...
Donate.tsx mount
   ▼
GetAllOrganizations()   [KarmaCircleApi.ts, GET /organizations]  →  organizationData
   ▼
