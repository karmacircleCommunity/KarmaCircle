...
2. Continue calls `CheckEmailExists(email)` (`KarmaCircleApi.ts`, `GET /auth/check-email`, no auth). If the backend reports the email already has an account, the flow advances to **`"signin"`**; otherwise it advances to **`"signup"`**. If the check itself fails or is unreachable, the flow fails open to `"signup"` — a genuine duplicate is still caught by the signup submit's own 409 (see `useAuth.ts` below).
