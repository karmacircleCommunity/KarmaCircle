...
3. Validates `credentials.password`, differently per `authType`:
   - **Sign-in:** only checks the field is non-empty (`"Password is required"` on failure). The strength rule below is a password-*creation* concept, not a login concept — actual credential correctness is left entirely to the backend's `LoginUser` response. (This used to reuse the sign-up strength check below and reject valid logins from any account whose password predated that rule; fixed.)
   - **Sign-up:** checks `credentials.password` against `passwordRegex` (`static/Constants.ts` — 8+ chars, at least one digit, one lowercase, one uppercase letter; exported so `SignUp.tsx`'s live strength meter can read the same rule, see below).
