...
The `"signin"` step's "Forgot password?" is a real link to `/auth/forgot-password`, and passes the email already entered on that step along in router `state` (not a query param — it's personal data) so the reset page opens with it pre-filled — see "Forgot password / reset password" below.
...
5. On `response.status` 200/201: shows a success toast, dispatches `updateUserData({ ...response.data.user, isLoggedIn: true })` to Redux, then clears `loading` and navigates.
   ...
   The navigate used to sit behind a fixed 1000ms `setTimeout` (to let the toast breathe); a later fix moved it to fire in the same tick as the dispatch instead, reasoning that a synchronous navigate would change the route (and unmount `Auth`) before `DonotRenderWhenLoggedIn`'s guard — which fires `<Navigate to="/" />` the instant Redux `isLoggedIn` flips true — ever got a chance to re-evaluate. **That didn't hold**: same-tick ordering of a `dispatch()` and a `navigate()` call doesn't guarantee which one's resulting render (or, worse, which one's effect) commits last, and `<Navigate>` fires its own redirect from an unguarded `useEffect` regardless of source order — instrumenting `history.pushState` showed the guard's `navigate("/")` landing a couple of milliseconds *after* this one, silently overwriting it. Fixed at the guard instead (see below): `DonotRenderWhenLoggedIn` now only redirects based on the `isLoggedIn` value it saw at mount, so a login/signup succeeding while it's already mounted can no longer race this navigate — this call is once again just a plain, unguarded `navigate()`, no ordering trick required. The app-level Toaster keeps the success toast on screen across the navigation regardless.
...
1. **`/auth/forgot-password`** ([apps/web/src/features/authentication/pages/ForgotPassword.tsx](../../apps/web/src/features/authentication/pages/ForgotPassword.tsx)) — reached from `Auth.tsx`'s `"signin"` step, which passes the already-entered email through router `state` so this page's email field starts pre-filled (a hand-typed visit to the route just starts empty). A single email field; on submit, `ForgotPassword(email)` (`KarmaCircleApi.ts`, `POST /auth/forgot-password`) always resolves the same way whether or not that email has an account (see [apps/api/docs/specs/auth.md](../../apps/api/docs/specs/auth.md)), so this page always swaps to a "check your inbox" confirmation on success rather than branching on account existence.
...
It redirects to `/` (`<Navigate to="/" replace />`) if the Redux `isLoggedIn` selector was truthy **at mount** — captured once into a `useRef` on the first render, not read live on every render. That distinction matters: `useAuth.ts`'s `authenticateUser` also flips `isLoggedIn` to `true` on a successful login/signup while this guard is already mounted (the user is sitting on `/auth/signin` or `/auth/signup`), and it already calls its own `navigate()` for that transition (to `/` or, for a new organization, `/organization/setup`). An earlier version of this guard read `isLoggedIn` live, so that same state flip also made it render `<Navigate to="/">` — and since `<Navigate>` fires its own redirect from an unguarded `useEffect`, the two navigations raced, with the guard's `navigate("/")` frequently landing a couple of milliseconds after `useAuth.ts`'s and winning, silently bouncing every new organization signup back to `/` instead of `/organization/setup` (see the note in "The flow, step by step" above). Reading `isLoggedIn` only at mount fixes this at the source: the guard's job is "redirect someone who *arrived* here already authenticated" (e.g. a Google OAuth redirect back to `/auth/signin` with the session cookie already set), not "react to every future authentication that happens while mounted" — the latter is already `useAuth.ts`'s job. One tradeoff worth knowing: if `isLoggedIn` flips true via some other means entirely while this page is sitting open (e.g. another tab logging in and a cross-tab storage sync) rather than through this page's own `useAuth` call, this guard will no longer catch that and redirect — an accepted, currently-unhit edge case.
