...
5. On `response.status` 200/201: shows a success toast, dispatches `updateUserData({ ...response.data.user, isLoggedIn: true })` to Redux, then clears `loading` and navigates — both in the same tick as the dispatch, deliberately not deferred.
   ...
   The navigate used to sit behind a fixed 1000ms `setTimeout` (to let the toast breathe), but `Auth` is wrapped in `DonotRenderWhenLoggedIn`, which fires `<Navigate to="/" />` the instant Redux `isLoggedIn` flips true — so the delay let that guard win the race and bounce every new organization to `/` for a second before the deferred `navigate("/organization/setup")` finally ran, a visible double redirect. Navigating synchronously changes the route (and unmounts `Auth`) before the guard re-evaluates; the app-level Toaster keeps the success toast on screen across the navigation regardless.
