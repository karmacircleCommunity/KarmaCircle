...
Purely data-driven from [src/components/footer/footerLinksConfig.ts](../../src/components/footer/footerLinksConfig.ts): `quickStarts`, `resources` (external links open in a new tab if the path starts with `http`), `policies` (`/terms`, `/privacy`, `/cookies` — none of these routes exist in `routesConfig.tsx`), and `social` (LinkedIn/X/GitHub icons, mapped through a local `icons` lookup keyed by the string names `FaLinkedinIn`/`FaXTwitter`/`FaGithub`). To add a footer link, edit `footerLinksConfig.ts` rather than this component.

## `Header`

[src/components/header/Header.tsx](../../src/components/header/Header.tsx) + [HeaderData.ts](../../src/components/header/HeaderData.ts).
...
[src/components/modal/Modal.tsx](../../src/components/modal/Modal.tsx).
...
[src/components/buttons/backtotopbutton/BacktoTop.tsx](../../src/components/buttons/backtotopbutton/BacktoTop.tsx).
...
[src/components/ClickAwayListener.tsx](../../src/components/ClickAwayListener.tsx).
...
[src/components/seo/ComponentHelmet.tsx](../../src/components/seo/ComponentHelmet.tsx).
