...
Redesigned August 2026 (lighter typography, the site's new muted-clay brand color — see [ui-kit.md](./ui-kit.md) — and the app's new Lenis/GSAP/three.js animation infra, also documented there): the old static `Vector.png` background is gone, replaced by `HeroScene.tsx`, a lazy-loaded three.js/`@react-three/fiber` static line grid (a same-day iteration replaced an initial random-particle-field version — see the feature's own `SPEC.md`). That grid's line opacity was raised from `0.08` to `0.16` in August 2026 after it was reported as invisible — it had never been removed, it had just never been perceptible: a WebGL `lineBasicMaterial` line is one *device* pixel wide (`linewidth` is a no-op in browsers), so at 2x DPR it's a half-CSS-pixel hairline, and the `dpr={[1, 1.5]}` cap resamples it softer still. Its CSS radial mask was widened at the same time so the grid isn't already half-faded by mid-hero. Don't compare its alpha against the CSS grids elsewhere in the app (`DrivesRail.tsx`, `OpenSource.tsx`) — those are crisp full-pixel lines and read much stronger at a lower number. The heading/paragraph/CTA fade in on mount and the whole hero block parallaxes on scroll, both via `@gsap/react`'s `useGSAP` + `ScrollTrigger`, skipped under `prefers-reduced-motion`. The `--landing-accent`/`--auth-accent`-style local color override this page used to carry is gone too — it now just uses the site-wide `--color-brand` tokens directly, since they're no longer the orange it was scoped away from.
