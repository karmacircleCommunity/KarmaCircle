...
[apps/web/src/features/clubs/pages/Clubs.tsx](../../apps/web/src/features/clubs/pages/Clubs.tsx).
...
The real data-fetching function, `getClubs()` in [apps/web/src/features/clubs/services/Clubs.ts](../../apps/web/src/features/clubs/services/Clubs.ts) (`GET /clubs` via `clubEndpoints.all`), exists but is never called by this page.
...
[apps/web/src/features/clubs/components/ClubCard.tsx](../../apps/web/src/features/clubs/components/ClubCard.tsx).
...
Not exported from the shared barrel (`apps/web/src/components/index.ts`) — only imported directly by `Clubs.tsx`.

## Types

Both files, plus `services/Clubs.ts`, are TypeScript; `types/index.ts` in this folder defines `Club`/`ClubCardProps`. See [clubs/SPEC.md](../../apps/web/src/features/clubs/SPEC.md#types) for the full breakdown, including how `UserType` is shared with `authentication`.
