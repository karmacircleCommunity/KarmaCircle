...
[src/features/events/pages/Events.jsx](../../src/features/events/pages/Events.jsx).
...
The real fetcher, `getEvents()` in [src/features/events/services/Events.js](../../src/features/events/services/Events.js) (`GET /events` via `eventEndpoints.all`), exists but is never called here.

The "Create An Event" button opens `CreateEvent` from [src/features/events/components/CreateEvent.jsx](../../src/features/events/components/CreateEvent.jsx).
...
[src/features/events/components/CreateEvent.jsx](../../src/features/events/components/CreateEvent.jsx).
...
[src/features/events/components/CreateEvents.jsx](../../src/features/events/components/CreateEvents.jsx).
A much more complete, MUI-based form: event name, MUI `DatePicker`/`TimePicker` (via `dayjs`) for start/end date+time, an event-mode `<Select>` (Online/Offline), a unique event ID (`uid`) field, description, a cover-image upload converted to base64 via [convertToBase64.js](../../src/features/events/utils/convertToBase64.js), and mode-dependent accordion sections: Offline shows city/state/address/country (`<Select>` populated from [static/CountryList.js](../../src/statics/CountryList.js))/map-iframe fields; Online shows a platform `<Select>` (populated from [static/OnlinePlatform.js](../../src/statics/OnlinePlatform.js): Zoom/Google Meet/Microsoft Teams/etc., each with an icon) and a platform-link field.
Validation and submission go through the [useEvent](../../src/features/events/hooks/useEvent.js) hook (see below), which does call the real `CreateEvent` API function (`MilanApi.js`, `POST /events/create`).
...
[src/features/events/hooks/useEvent.js](../../src/features/events/hooks/useEvent.js) — pairs with `CreateEvents` (the MUI one, not `CreateEvent`).
...
- [EventsMarqueeCards.jsx](../../src/features/events/components/EventsMarqueeCards.jsx) — takes an `event` prop, renders cover image, name, and either a location (Offline) or a platform icon+name (Online), plus a formatted start date/time (via [getFormattedDate.js](../../src/features/events/utils/getFormattedDate.js)). Responsive text truncation at `window.innerWidth <= 500`. Not currently rendered by any page — looks intended for a "recent events" marquee (there's a commented-out `<Marquee>` block in `Profile.jsx` that would have used something like this).
- [EventCard.jsx](../../src/features/events/components/EventCard.jsx) — the card rendered in the `Events.jsx` grid. **Does not use its data at all**: no props are accepted or destructured; every field (title "Food Marathon, 2025", club name, description, avatar images, "+300 Participated") is hardcoded JSX. `Events.jsx` passes `event={event}` to it, but the prop is ignored.
- [EventSlider.jsx](../../src/features/events/components/EventSlider.jsx) — a custom (non-Swiper) auto-advancing carousel over a hardcoded array alternating `FeaturedEventImage`/`FeaturedEventCard`, paired two-per-slide. Advances every 3s via `setInterval`.
- [FeaturedEventCard.jsx](../../src/features/events/components/FeaturedEventCard.jsx) / [FeaturedEventImage.jsx](../../src/features/events/components/FeaturedEventImage.jsx) — also fully static/hardcoded, no props.
...
- [src/features/events/components/HostedEvents.jsx](../../src/features/events/components/HostedEvents.jsx) — file exists but is **completely empty** (0 bytes); importing it would fail.
- [src/features/events/pages/DetailedEvent.jsx](../../src/features/events/pages/DetailedEvent.jsx) — a one-line placeholder (`<div>DetailedEvent</div>`), not registered in `routesConfig.jsx` (no `/events/:id`-style route exists).
