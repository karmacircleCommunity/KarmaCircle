...
- **"Maybe later" still exists, quietly.** Setup is not technically mandatory and the flow resumes from the navbar, so the link stays (`data-cy="setup-later"`) — but as a small, centred `text-caption` text link under the CTA, not a second button beside it. The "leaving now is fine, here's how to get back" paragraph is gone. The de-emphasis has a floor, though: `text-ink/65` is the minimum opacity at this 10px `text-caption` size (this is normal-size text for WCAG's 4.5:1 contrast rule, not "large text"'s 3:1 — a lighter `/40`, tried first, measured 2.36:1 against the cream background and failed AA). The button also carries padding offset by an equal negative margin, so its hit area clears the 24×24 AA touch-target minimum without visually taking up more space than the text itself — this is the second time this exact link has been pushed too far past its own de-emphasis, so don't lighten it further without rechecking contrast.
...
- **A required question cannot be walked past, but the flow can be left at any time.** These are not the same freedom, and the distinction is the whole design: "Save and finish later" (and "Maybe later" on the intro) writes whatever is filled in and goes, while Continue refuses to move off a required question that is blank. An answer skipped mid-flow is one nobody would ever be prompted for again — the organization would sit in draft with no idea which screen it was on.
  **Drifted from its own original design — see [known-issues.md](./known-issues.md#smaller-one-off-issues).** Continue is now `disabled` (`disabled={saving || !canContinue}`) whenever a required question is blank, so it can never actually be pressed while blocked — the described "refusal focuses the offending field and turns the note under it red" (`setBlockedOn`, the `setup-required-note` element) is dead code, unreachable through the UI. The only explanation left is a native `title` tooltip ("Fill this in to continue."), which is not screen-reader-visible the way the red note was.
...
- **The three contact answers are checked for shape before a step saves** (`validateSetupField` / `invalidFields` in `utils/organizationSetupForm.ts`), and Continue refuses on a malformed one exactly as it does on a blank required one.
  This is a different objection from "you left something out": all three fields are optional, so blank is fine and wrong is not, and the message sits under the offending field rather than in the one shared note.
  Website wants a host with a dot and a real TLD - which also catches what `normalizeWebsite` would otherwise turn into `https://karmacircle`, a string the backend's `.url()` accepts; email wants the typo-level check (`@`, a dot, no spaces, 254 chars), not RFC 5322; phone wants 7-15 digits, E.164's own range.
  The alternative is a 400 that arrives after the save, on a screen that has already scrolled away, saying "Invalid url" without saying which field it means.
- **The phone field is sanitised as it is typed** (`sanitizeSetupValue`), keeping digits, separators and a single leading `+`.
  It used to take `8245034+======` happily and only object at save time; a character that can never be part of the answer is better dropped at the keystroke than explained afterwards.
  The same 7-15 digit rule is enforced server-side too (`CONTACT_PHONE` in the API's `organization.validation.ts`) - the client's check is for the message, not for the guarantee.
- **A format error waits for the blur**, or for a refused Continue (`showErrors`), because every email address is invalid until the moment it isn't and complaining mid-word is noise.
  Leaving the flow is subject to the same check, since the exit saves: a malformed answer has to be fixed or cleared first, rather than being dropped silently or ending in a 400 nobody is on screen to read.
...
The whole journey is covered end to end by [`e2e/organization-setup.spec.ts`](../../apps/web/e2e/organization-setup.spec.ts) (Playwright, replacing the Cypress spec of the same purpose) — sign up, skip, come back, save each step, go live. See [testing.md](./testing.md) for how it runs. The `cypress/walkthrough/organizationJourney.spec.js` demo-recording spec that used to accompany it was retired with the Cypress→Playwright migration rather than ported — Playwright's own trace viewer and `--headed`/video output (`CAPTURE_TOUR=1 pnpm test`, see `e2e/organization-setup.spec.ts`'s `tour()` helper) cover the same "watch it happen" need without a second, redundant spec file.
...
The page is scoped by `useSectionReveal(pageRef, [organization.userName])`. It used to be called with no dependency array, which left every `data-reveal` element (name, tagline, meta row, stats, both sidebar cards) stuck at its pre-animation opacity indefinitely — reproduced on both a client-side redirect straight from "Save and publish" and a hard reload, so it wasn't a first-paint-vs-navigation timing fluke. This page's content arrives from the `GET /organizations/{handle}` fetch above, the same "async-loaded content needs a dependency array" case `Organizations.tsx`/`Events.tsx` already guard against — see [ui-kit.md](./ui-kit.md#motion). A user landing here right after finishing setup — the best moment for a first impression — was seeing a near-blank page.
