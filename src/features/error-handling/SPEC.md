...
The 404 catch-all page, plus one leftover dev-scratch file (`Test.jsx`) that happens to still live in the tree.
...
| File | Role | Live? |
|---|---|---|
| `pages/Error404.jsx` | The `*` catch-all route's component | ✅ routed |
| `pages/Error404.css` | Styles | ✅ |
| `pages/Test.jsx` | Dev leftover — redirects to Google on mount | ❌ not routed, unreachable, but still present in the tree |
...
## `pages/Test.jsx` — dev leftover, unreachable but present
...
- No `<Helmet>`/SEO tags on `Error404.jsx` — already in `known-issues.md` in spirit (via the general SEO-convention discussion), called out explicitly here.
- `Error404.jsx` renders no `Navbar`/`Footer` — new observation, confirm intentional before treating as a bug.
- `Test.jsx` is dev-scratch code, safe to delete once confirmed — already in `known-issues.md`.
...
- **"Make the app not go blank on errors"** → add a top-level error boundary in `src/app/App.jsx` around `<Routes>`; this folder is a reasonable home for the fallback-UI component (visually, it could reuse `Error404.jsx`'s illustration/button pattern for a consistent "something went wrong" look).
- **"Clean up dead files"** → `pages/Test.jsx` is the clearest candidate in this folder; confirm before deleting per the guidance above.
