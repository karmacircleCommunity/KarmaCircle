...
- Don't assume `clubEndpoints.details(userName)` is club-only. It's queried for both individual and club profiles today (`Profile.tsx` uses it for `/user/:userName` and `/club/:userName` alike), despite the name.
