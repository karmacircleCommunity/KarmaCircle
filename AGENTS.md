...
- Test tooling: `apps/web` has a Playwright E2E suite (`apps/web/e2e/`, `pnpm test`) — full-stack, a real browser against a real (isolated) API — but no component/unit-test runner. `apps/api` has Jest + Supertest (`npm test`), with every module covered. `pnpm test` from the repo root runs both suites at once via Turborepo. See [docs/specs/testing.md](./docs/specs/testing.md) (`apps/web`) and [apps/api/docs/specs/testing.md](./apps/api/docs/specs/testing.md) for the full picture. Don't claim something is "tested" without running it through the relevant tooling or manually verifying.
